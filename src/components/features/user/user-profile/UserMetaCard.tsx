...
export default function UserMetaCard() {
  ...
  const { username, firstname, lastname, email, roles } = decodedToken || {};
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {roles?.length > 0
                    ? roles.map((role, index) => (
                        <span
                          key={index}
                          className="inline-block px-2 py-1 text-xs font-medium text-gray-800 bg-gray-200 rounded-full dark:bg-gray-700 dark:text-white"
                        >
                          {role}
                        </span>
                      ))
